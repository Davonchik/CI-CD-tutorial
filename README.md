...
CI/CD – это подход к разработке ПО, при котором рутинные процессы сборки, тестирования и развертывания автоматизированы. Процесс CI/CD часто называют конвейером (pipeline), где исходный код на входе превращается в готовое приложение на выходе. Такой конвейер выступает в роли «робота-помощника», который берёт на себя скучные задачи: запускает тесты, собирает проект, выкладывает его на сервер ￼. Цель CI/CD – ускорить и упростить доставку изменений в программном обеспечении, обеспечивая высокое качество и надежность релизов.

На схеме показан общий процесс CI/CD: этапы непрерывной интеграции (Build, Test, Merge) плавно переходят в этапы непрерывной доставки (Release) и непрерывного развертывания (Deploy). Непрерывная интеграция позволяет автоматически собрать и проверить код, а непрерывная доставка подготавливает его к релизу. При реализации непрерывного развертывания деплой на рабочий сервер происходит автоматически после прохождения всех проверок.
...
Каждый коммит в репозиторий может запускать автоматическую сборку проекта и серию тестов. Это позволяет сразу обнаружить, если новое изменение сломало сборку или тесты ￼. Основные черты CI:
